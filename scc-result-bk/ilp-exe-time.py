...
def parse_one_file(filepath):
    ...
    line_re = re.compile(
        r"/scc-500/(\d+)/benchmark_[^:]+:\s*\[([0-9]*\.?[0-9]+)\]"
    )
...
def plot_multiple_files(pattern="scc_times-*.txt", output_file="ilp_times_multi.png"):
...
if __name__ == "__main__":
    plot_multiple_files(pattern="scc_times-*.txt", output_file="ilp_times_multi.png")