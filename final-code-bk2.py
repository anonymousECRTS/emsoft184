...
class MimosNetwork:
    ...
    def sccs_in_topo_order(self) -> List[List[int]]:
        ...
        # Pass 1: finishing order (iterative DFS)
        ...
        # Pass 2: SCC extraction on transpose (iterative DFS)
        visited = {v: False for v in self.nodes}
        sccs: List[List[int]] = []
    ...
    def _scc_ilp_feasible(self, comp: List[int]) -> bool:
        ...
        Lower bound tightening:
          since x(i) >= 0 and A_p >= 0, for any selected incoming edge p of j:
              floor(A_p / W_p) <= x(j)
          hence we can safely use
              lb_j = min_{internal incoming p to j} floor(A_p / W_p)
        """
        try:
            import pulp
        ...
        for e in self.edges.values():
            if e.source in comp_set and e.target in comp_set:
                ...
                y[e.id] = pulp.LpVariable(
                    f"y_{e.id}", lowBound=0, upBound=1, cat=pulp.LpBinary
                )
        ...
        # Lower bounds: lb[j] = min floor(A_p / W_p) over internal incoming edges of j
        lb = {
    ...
    def analyze_execution(
        ...
        print(f"undoundedness time: {ub_t1 - ub_t0:.6f} s")
