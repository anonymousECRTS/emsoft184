...
class MimosNetwork:
    ...
    # ------------ ILP / SMT feasibility for SCC inequalities ------------

    def _scc_ilp_feasible(
        self,
        comp: List[int],
        solver: str = "z3",
    ) -> bool:
        ...
        if solver == "z3":
            if z3 is None:
                raise RuntimeError("Z3 is required for solver='z3': pip install z3-solver")
            ...
            for j in comp:
                s.add(z3.PbEq([(y[e.id], 1) for e in incoming_internal[j]], 1))
            ...
            print(f"Z3 time: {ilp_t1 - ilp_t0:.6f} s")
            ...
            try:
                ...
                raise RuntimeError("PuLP is required for solver='pulp': pip install pulp") from e
            ...
            ilp_t1 = time.perf_counter()
            print(f"PuLP time: {ilp_t1 - ilp_t0:.6f} s")

            return pulp.LpStatus[prob.status] == "Optimal"

        else:
            raise ValueError(f"Unknown solver '{solver}'. Use 'z3' or 'pulp'.")
    ...
    def detect_blocked_nodes(self, solver: str = "z3") -> Set[int]:
        blocked: Set[int] = set()
        ...
        for comp in sccs_ordered:
            ...
            if has_incoming_from_blocked(comp):
                ...
                if self._scc_ilp_feasible(comp, solver=solver):
                    blocked.update(comp)

        return blocked

    def detect_deadlock(self, solver: str = "z3") -> Set[int]:
        return self.detect_blocked_nodes(solver)
    ...
    def _queue_value_from_x(self, x_hist: Dict[int, Dict[int, int]], eid: int, t: int) -> int:
        """
        Queue occupancy at time t under the paper's convention:
          Q_p(t) = A_p + U_p * x(i, t - P_i) - W_p * x(j, t - 1)

        Meaning:
          - after all writes visible at time t,
          - before any consumptions at time t.
        """
        e = self.edges[eid]
        Pi = self.nodes[e.source].period
        xi_term = self._get_x(x_hist, e.source, t - Pi)
        xj_term = self._get_x(x_hist, e.target, t - 1)
    ...
    def isolated_scc_effective_periods(self, comp: List[int], max_hyper: int = 200) -> Dict[int, float]:
        ...
        # q represents Q_p(t): after writes at time t, before any reads at time t.
        ...
        for t in event_times:
            if t == 0:
                ...
                for j in released:
                    ...
                    if not incoming or all(q[eid] >= self.edges[eid].W_p for eid in incoming):
                        will_fire.append(j)
    ...
    def exact_queue_bounds_for_bounded_execution(
        ...
        """
        Exact bounded analysis under write-before-read semantics.

        Q_p(t) is interpreted as the queue occupancy
        after writes visible at time t and before any consumptions at time t.

        Event order at each time t:
          1) apply writes visible at time t,
          2) evaluate eligibility of all released nodes using the pre-consumption queues,
          3) consume for all nodes that fire at time t,
          4) schedule their future writes.
        """
        H = self.hyperperiod()

        # q always represents the current pre-consumption queue state
        # at the present event time: after writes, before reads.
        ...
        if store_history:
            ...
            for j in self.nodes:
                ...
                alpha_hist[j][0] = firings[j] / max(1, (0 // base) + 1)
        else:
            q_hist = None
            fire_hist = None
            alpha_hist = None
        ...
        def record_time(t: int):
            # Records the paper's Q_p(t): after writes at time t, before reads at time t.
    ...
    def analyze_execution(
        ...
        blocked = self.detect_blocked_nodes(solver=solver)
    ...
    def simulate_event_driven(
        ...
        """
        Independent event-driven simulation under write-before-read semantics.

        At each time t:
          1) apply writes that become visible at t,
          2) evaluate eligibility using queue occupancies after writes and before reads,
          3) consume tokens for all nodes that fire at t,
          4) schedule writes for future completion times.
        """
        ...
        # q represents the paper's Q_p(t): after writes at t, before reads at t.
        ...
        if verbose:
            print(f"[sim] m=0, t=0, sampled_state={state0}")
        ...
        for t in event_times:
            if t == 0:
                ...
                if verbose:
                    print(f"[sim] t=0, released={released}, fired={will_fire}")
                continue
            ...
            if verbose:
                released_dbg = [j for j in sorted(active_nodes) if t % self.nodes[j].period == 0]
                print(f"[sim] t={t}, writes={visible_writes}, released={released_dbg}")
            ...
            if verbose:
                print(f"[sim] t={t}, fired={will_fire}")
...
def plot_queues_separate(
    ...
    for eid, hist in q_hist.items():
        ...
        ax.set_ylabel(rf"$Q_{{{eid+1}}}(t)$", fontsize=18)
        ax.grid(True, which="both", linestyle=":")
        ax.tick_params(axis="both", labelsize=16)
        ymax = max(vals)
        ax.set_yticks(range(0, int(math.ceil(ymax)) + 4, 2))
        ax.set_ylim(0, int(math.ceil(ymax)) +4)
...
def plot_alpha_separate(
    ...
    for nid, hist in alpha_hist.items():
        ...
        _add_transient_steady_markers(ax, t0=t0, T_sys=T_sys)
        ax.set_xlabel("time", fontsize=18)
        ax.set_ylabel(rf"$\alpha({nid+1},t)$", fontsize=18)
        ax.tick_params(axis="both", labelsize=16)
        ax.grid(True, which="both", linestyle=":")
        ax.set_ylim(0, 1.0)
...
def example1(
    ...
    if run_sim:
        ...
        compare_analysis_with_event_simulation(
            net,
            max_hyper=200,
            verbose=verbose,
            solver=solver,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="z3",
        choices=["z3", "pulp"],
        help="Solver for blocked-node feasibility",
    )
